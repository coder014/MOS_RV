...
int sys_mem_map(u_int srcid, u_int srcva, u_int dstid, u_int dstva, u_int perm) {
	...
	if(!(pp=page_lookup(srcenv->env_pgdir, srcva, NULL))) return -E_INVAL;
...
int sys_ipc_try_send(u_int envid, u_int value, u_int srcva, u_int perm) {
	...
	e->env_ipc_perm = PTE_U | PTE_R | PTE_V | perm;
	e->env_ipc_recving = 0;
	...
	if (srcva != 0) {
		...
		try(page_insert(e->env_pgdir, e->env_asid, p, e->env_ipc_dstva, perm | PTE_U));
	}
	return 0;
}
